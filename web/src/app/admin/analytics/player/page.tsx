...
export default function PlayerAnalyticsPage() {
  ...
  async function handleFetch() {
    ...
    setError(null);
    try {
      const result = await getPlayerAnalytics(sport, playerId.trim());
      setData(result);
    } catch (err) {
  ...
  return (
    <div className={styles.container}>
      <header className={styles.pageHeader}>
        ...
        <p className={styles.pageSubtitle}>View individual player metrics and derived stats</p>
      </header>

      <AdminCard title="Search Player">
