"""NCAAB game-level model.

Predicts game-level outcomes (win probability, expected scores) from
team-level four-factor features. Can use a trained model or fall back
to a rule-based approach using offensive and defensive ratings.

Usage::

    model = NCAABGameModel()
    pred = model.predict({"home_off_rating": 110.0, "away_off_rating": 100.0, ...})
    # -> {"home_win_probability": 0.72, "expected_home_score": 78.5, ...}
"""
...
_DEFAULT_HOME_WP = 0.67  # NCAAB has strong home-court advantage
_DEFAULT_HOME_PTS = 74.0
_DEFAULT_AWAY_PTS = 68.0


class NCAABGameModel(BaseModel):
    """Predicts NCAAB game outcomes from four-factor team features.

    With a trained model loaded, uses it to predict win probability
    and expected scores. Without a trained model, uses a simple
    home-advantage baseline with four-factor adjustments.
    """
    ...
    def _predict_rule_based(self, features: dict[str, Any]) -> dict[str, Any]:
        """Simple home-advantage baseline with four-factor adjustments."""
        wp = _DEFAULT_HOME_WP
