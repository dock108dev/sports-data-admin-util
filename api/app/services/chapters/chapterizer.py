...
class Chapterizer:
        ...
        for i, play in enumerate(plays):
            ...
            if self.rules.is_period_start(event, prev_event):
                ...
                self.debug_logger.log_boundary_triggered(
                    ...
                    rule_precedence=BOUNDARY_PRECEDENCE.get(
                        BoundaryReasonCode.PERIOD_START, 0
                    ),
            ...
            if self.rules.is_overtime_start(event, prev_event):
                ...
                self.debug_logger.log_boundary_triggered(
                    ...
                    rule_precedence=BOUNDARY_PRECEDENCE.get(
                        BoundaryReasonCode.OVERTIME_START, 0
                    ),
                    boundary_action=BoundaryAction.START_NEW,
                )
            ...
            if is_timeout or is_review:
                ...
                if self.config.collapse_reset_clusters:
                    if reset_cluster_start is None:
                        ...
                        if (
                            i - reset_cluster_start
                            <= self.config.reset_cluster_window_plays
                        ):
            ...
            if self.rules.is_crunch_start(event, prev_event, context):
                ...
                self.debug_logger.log_boundary_triggered(
                    ...
                    rule_precedence=BOUNDARY_PRECEDENCE.get(
                        BoundaryReasonCode.CRUNCH_START, 0
                    ),
                    boundary_action=BoundaryAction.START_NEW,
                )
        ...
        for boundary in sorted_boundaries:
            ...
            if chapter_plays:
                ...
                if chapter_id == 1 and current_start_idx == 0:
                    ...
                    reason_codes = boundary_reasons.get(
                        current_start_idx, ["PERIOD_START"]
                    )
    ...
    def _extract_period_and_time(
        self, plays: list[Play]
    ) -> tuple[int | None, TimeRange | None]:
        ...
        time_range = (
            TimeRange(start=start_clock, end=end_clock)
