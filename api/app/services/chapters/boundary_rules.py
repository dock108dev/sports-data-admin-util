...
class BoundaryMarker:
    ...
    def to_dict(self) -> dict[str, Any]:
        ...
        return {
            ...
            "score_snapshot": {
                "home": self.score_snapshot[0],
                "away": self.score_snapshot[1],
            },
        }
...
class NBABoundaryRules:
    ...
    def is_overtime_start(
        ...
        return (
            curr_quarter is not None
            and curr_quarter > 4
            and prev_quarter is not None
            and prev_quarter == 4
        )
    ...
    def is_review(event: dict[str, Any]) -> bool:
        ...
        return (
            ...
            or "challenge" in description
            or "review" in play_type
            or "challenge" in play_type
        )
...
def resolve_boundary_precedence(
    reason_codes: list[BoundaryReasonCode],
) -> list[BoundaryReasonCode]:
    ...
    sorted_codes = sorted(
        reason_codes, key=lambda code: BOUNDARY_PRECEDENCE.get(code, 0), reverse=True
    )
    ...
    for code in sorted_codes:
        ...
        if code in (
            BoundaryReasonCode.PERIOD_START,
            BoundaryReasonCode.PERIOD_END,
            BoundaryReasonCode.OVERTIME_START,
            BoundaryReasonCode.GAME_END,
        ):
