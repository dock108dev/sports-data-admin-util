...
def _find_weighted_split_points(
    ...
    # =========================================================================
    # FIX 1: Apply late-game amplifier BEFORE any allocation
    # Late drama needs to be overweighted - this is narrative truth
    # Sport-aware: NBA=quarters, NCAAB=halves, NHL=periods
    # =========================================================================
    amplified_weights: dict[str, float] = {}

    # Determine amplification based on sport structure
    if league_code == "NCAAB":
        # NCAAB uses halves (H1, H2 internally stored as Q1, Q2)
        # H1 = early game (suppress), H2 = late game (boost strongly)
        # User insight: last 10 min of H2 = Q4 equivalent
        ...
        # P3 = late game, boost. P1 = early, suppress.
        for q in sorted_quarters:
            base_weight = quarter_weights.get(q, 1.0)
    ...
    median_weight = sorted(weights_list)[len(weights_list) // 2]

    logger.info(
        f"Peak quarter: {peak_quarter} (weight={peak_weight}), "
        f"median_weight={median_weight}"
    )

    # =========================================================================
    # STEP 2: Allocate blocks per quarter using floor() + deficit backfill
    # =========================================================================
    # Total blocks to distribute (SETUP and RESOLUTION are structural, not allocated)
    # We allocate ALL target_blocks, then enforce Q1 cap
    available_blocks = target_blocks
    ...
    # =========================================================================
    # STEP 3: Back-fill deficit by drama priority (NOT evenly)
    # Sort by: 1) higher weight, 2) later chronological order (back-bias)
    # CRITICAL: Exclude Q1 from backfill - it should never receive spillover
    # =========================================================================
    # Q1 exclusion determined early (before we know q1_is_dramatic from step 4)
    # Use a strict threshold: Q1 only eligible if it's THE peak quarter
    ...
    # Q1 gets max 1 block unless it has EXCEPTIONAL drama (top quartile)
    # This is strict: early lead changes and scoring runs don't count
    # =========================================================================
    q1_weight = quarter_weights.get("Q1", 1.0)  # Already suppressed by 0.8x
    # Q1 is only "dramatic" if it's in the top 25% of weights AFTER amplification
    # This should be rare (injury, ejection, record event, massive blowout start)
    ...
    # =========================================================================
    # STEP 5: Place splits WITHIN each quarter locally (not globally)
    # =========================================================================
    split_points: list[int] = []
    ...
    for q_key in sorted_quarters:
        ...
        # internal_splits = blocks_for_quarter - 1
    ...
    # =========================================================================
    # STEP 7: If still short, add splits in high-drama quarters (back-biased)
    # NO GLOBAL EVEN SPACING - this is the key fix
    # =========================================================================
...
def _calculate_swing_metrics(block: NarrativeBlock) -> dict[str, int]:
...
def _assign_roles(blocks: list[NarrativeBlock]) -> None:
    ...
    for i, block in enumerate(blocks):
        ...
        metrics = _calculate_swing_metrics(block)
...
async def execute_group_blocks(stage_input: StageInput) -> StageOutput:
    ...
    if is_blowout:
        ...
        if quarter_weights:
            ...
            split_points = _find_weighted_split_points(
