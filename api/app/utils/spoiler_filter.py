...
def filter_spoiler_posts(posts: list[dict], text_key: str = "text") -> list[dict]:
    """
    Filter a list of posts to remove those containing spoilers.

    Args:
        posts: List of post dictionaries
        text_key: Key in each dict containing the text to check

    Returns:
        List of posts without spoilers
    """
    return [
        post for post in posts
        if not contains_spoiler(post.get(text_key, ""))
    ]
