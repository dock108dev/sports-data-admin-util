...
def test_section_count_within_bounds(minimal_timeline, mock_ai_client):
    ...
    assert len(result.sections) >= 3, (
        f"Expected at least 3 sections, got {len(result.sections)}"
    )
    assert len(result.sections) <= 10, (
        f"Expected at most 10 sections, got {len(result.sections)}"
    )
...
def test_sections_are_ordered(minimal_timeline, mock_ai_client):
    ...
    for i, section in enumerate(result.sections):
        assert section.section_index == i, (
            f"Section {i} has wrong index: {section.section_index}"
        )
...
def test_validation_is_invoked(minimal_timeline, mock_ai_client):
    ...
    with (
        patch("app.services.chapters.pipeline.validate_pre_render") as mock_pre,
...
def test_pipeline_fails_loud_on_empty_timeline():
    ...
    assert (
        "build_chapters" in exc_info.value.stage.lower()
        or "no chapters" in str(exc_info.value).lower()
    )
