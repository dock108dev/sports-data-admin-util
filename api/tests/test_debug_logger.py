...
def test_boundary_trigger_logged():
    ...
    period_starts = [e for e in boundary_events if "PERIOD_START" in e.reason_codes]
...
def test_boundary_trigger_correct_play_index():
    ...
    timeout_events = [e for e in boundary_events if "TIMEOUT" in e.reason_codes]
...
def test_chapter_start_end_match_chapters():
    ...
    for chapter in story.chapters:
        matching_starts = [
            e for e in start_events if e.chapter_id == chapter.chapter_id
        ]
...
def test_logging_deterministic():
    ...
    chapterizer1.chapterize(timeline, game_id=1, sport="NBA")
    events1 = chapterizer1.debug_logger.get_events()

    chapterizer2 = Chapterizer(debug=True)
    chapterizer2.chapterize(timeline, game_id=1, sport="NBA")
