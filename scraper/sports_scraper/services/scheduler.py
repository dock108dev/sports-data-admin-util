...
def run_social_ingestion_for_league(league_code: str) -> dict:
    """Run social collection for a single league.

    Called as a scheduled task to collect X/Twitter posts for games.

    Args:
        league_code: The league to collect social for (NBA, NHL)

    Returns:
        Dict with social_posts count and games_processed
    """
    from ..social import XPostCollector
    from .game_selection import select_games_for_social
    from ..config_sports import is_social_enabled

    if not is_social_enabled(league_code):
        logger.info(
            "social_ingestion_disabled",
            league=league_code,
        )
        return {"league": league_code, "social_posts": 0, "status": "disabled"}
    ...
    with get_session() as session:
        ...
        for game_id in game_ids:
            try:
                results = collector.collect_for_game(session, game_id, is_backfill=False)
                for result in results:
                    social_posts += result.posts_saved
                games_processed += 1
