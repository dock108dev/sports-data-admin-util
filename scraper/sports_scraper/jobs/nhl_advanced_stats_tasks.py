"""Celery task for ingesting NHL MoneyPuck-derived advanced stats."""
...
@shared_task(
    name="ingest_nhl_advanced_stats",
    autoretry_for=(Exception,),
    retry_backoff=True,
    retry_kwargs={"max_retries": 3},
    default_retry_delay=300,
)
def ingest_nhl_advanced_stats(game_id: int) -> dict:
    """Ingest MoneyPuck-derived advanced stats for an NHL game.
    ...
    from ..services.nhl_advanced_stats_ingestion import ingest_advanced_stats_for_game

    job_run_id = start_job_run("ingest_nhl_advanced_stats", [])
    try:
